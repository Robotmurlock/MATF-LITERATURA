...
\section{Objasniti princip određivanja koraka prema rokovima.}
  ...
  \textbf{Dobit:}
  \begin{itemize}
    \item Drastično smanjivanje rizika od prekoračivanje budžeta ili rokova;
...
\section{Navesti dijagrame interakcija.}
  \begin{enumerate}
    ...
    \item Dijagram iterakcija (iteraction diagram ili interaction overview diagram)
...
\section{Kakav je odnos refaktorisanja i pisanja programskog koda?}
  ...
  lakši za održavanje i kako bi se povećala efikasnot pisanja koda. 
  ...
  a da se pritom ponašanje programe ne menja (menje se struktura). Refaktorisanje ne menja
...
\section{Šta su osnovni motivi za refaktorisanje koda?}
  ...
  \begin{itemize}
    ...
    \item Refaktorisanje podiže brzinu pisanja koda.
  \end{itemize}
...
\section{Zašto je dobro eliminisati ponavljanja iz koda? (refaktorisanje)}
  ...
  \begin{itemize}
    ...
    \item Povlačenje metoda kroz hijearhiju (dve klase imaju imaju iste atribute, ti atributi
...
\section{Zašto dugački metodi mogu predstavljati problem? (refaktorisanje)}
  ...
  \textbf{Simptomi:}
  \begin{itemize}
    \item Metod je predugačak. Za svaki metod koji je duži od deset linije je potrebno
          razmisliti od refaktorisanju.
  \end{itemize}

  \noindent \textbf{Uzrok:} 
  \begin{itemize}
    \item Konstatno dodavanje koda (dve po dve linije).
  \end{itemize}
...
\section{Šta su divergentne promene? Zašto su problematične? (refaktorisanje)}
  ...
  \noindent \textbf{Uzrok:} 
  \begin{itemize}
    \item Loša struktura programa ili ,,copypasta programming``.
  \end{itemize}
...
\section{Zašto velika zavisnost neke klase ili metoda od drugih klasa može da 
  ...
  \begin{itemize}
    \item Klasa ili metod koristi veliki broj metoda druge klase za čitanje vrednost.
  \end{itemize}
...
\section{Zašto naredba switch može da predstavlja problem? (refaktorisanje)}
  ...
  \begin{itemize}
    \item Izdvajanje metoda.
    \item Premeštanje metoda.
    \item Zamena kodiranog podataka potklasom (kada ima dosta operacija nad nekim kodiranim tipom
...
\section{Šta bi trebalno da sadrži opis svakog od refaktorisanja u katalogu?}
  ...
  \begin{itemize}
    ...
    \item Motivacija, koja koriste se može očekivati nakon primenjene tehnike.
...
\section{Navesti bar 5 grupa tehnika refaktorisanja.}
  ...
  \begin{itemize}
    ...
    \item \textbf{Organizovanje podataka:} Koriste se u okviru klasa da povećanju njenu
...
\section{Šta je dizajn softvera? Šta je arhitektura softvera? Objasniti sličnosti i razlike.}
  ...
  tome što je arhitektura softvera posmatrana na relativno visokom nivou. To znači da je
  svaka arhitektura softvera dizajn softvera, ali ne važi obrnuto. \\
  ...
  Pri pravljenju arhitekture softverskog sisteme preduzimaju se neke specifične aktivnosti i
...
\section{Navesti i ukratko objasniti obaveze softverskog arhitekte.}
  \begin{enumerate}
    ...
    \item \textbf{Upravljanje rizicima i promenama:} Odnosi se na terativno unapređenje i 
...
\section{Objasniti komunikacionu koheziju u kontekstu razvoja softvera.}
  ...
  \end{itemize}
  Primer: Ažurira se se slog u bazi podataka i šalje se pisaču.
...
\section{Objasniti proceduralnu koheziju u kontekstu razvoja softvera.}
  ...
  \begin{itemize}
    \item Jedna komponenta ma više odgovornosti;
...
\begin{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \textbf{Podeljene odgovornosti:} Poželjno je razmotriti učauravanje elemenata spoljašnjeg
          koncepta u novu komponentu (ili komponente), koju i zatim koristile ovako spregnute 
          komponente.
  \end{itemize}
...
\section{Objasniti pojam proces.}
  ...
  Obično se razmatra samo u kontekstu računarski sistema koji imaju mogućnost 
  ...
  \begin{itemize}
    \item Podaci koji se vode na nivou procesa i zajednički su za sve niti tog procesa obuhvataju:
      \begin{itemize}
        ...
        \item Informacije o memoriji (tablica stranica, podaci o alokaciji memorije, ...);
        \item Deskriptori datoteka;
        \item Stanje izvršavanje procesa;
...
\section{Objasniti kako se programiraju niti pomoću biblioteke QT. Klase, metodi, ...}
  ...
  \begin{itemize}
    ...
    \item \textit{bool wait(unsigned long time msec = ULONG MAX)}
  \end{itemize}
...
\section{Objasniti detaljno operaciju dovršavanja niti. Kako se implementira pomocu biblioteke QT?}
  ...
  Dinamički napravljen objekat će biti automatski uklonjen ako mu se pri konstrukciji 
  dodeli roditeljski objekat.
...
\section{Objasniti detaljno operaciju čekanja niti. Kako se implementira pomoću standardne
  ...
  da se pozove dva puta metod \textit{join()}. 
...
\section{Objasniti detaljno operaciju čekanja niti. Kako se implementira pomoću biblioteke QT?}
  ...
  funkciju. Nit se može čekati pozivanjem metoda bool \textit{wait(unsigned long time = ULONG MAX)}. 
  Ako se argument ne navede (ili se navede ULONG MAX), metod čeka dok se nit ne završi. 
...
\section{Objasniti šablone future i promise standardne biblioteke C++-a}
  Šablonska klasa \textit{promise} omogućava čuvanje vrednost ili izuzetka koji su
  kasnije preuzeti asinhrono preko \textit{future} objekta koji je kreiran preko
  ...
  \begin{itemize}
    \item \textbf{Mera napretka:}
      \begin{itemize}
        ...
        \item Na kvalitet utiče ujednačenost celicine zadataka.
      \end{itemize}
    \begin{figure}[H]
      \begin{center}
    ...
    \item \textbf{Mera završavanja:} Umesto ostvarenih poslova se predstavljaju preostali poslovi
          u odnosu na vreme. Ima bar dve linije tj. plan i ostvarivanje.
    \begin{figure}[H]
      \begin{center}
  ...
  \begin{itemize} 
    ...
    \item Poznavanje alata:
      \begin{itemize}
        ...
        \item asembler i masinski jezik
